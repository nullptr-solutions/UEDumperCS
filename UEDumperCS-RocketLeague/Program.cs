...
using UEDumperCS_RocketLeague.Engine;
...
var game = Array.Find(Process.GetProcessesByName("RocketLeague"), x => !string.IsNullOrEmpty(x.MainWindowTitle));
if (game is null)
{
    Logger.Get.Error("Could not find rocket league");
    return;
}

Logger.Get.Info($"Found rocket league! {{ pid: {game.Id}, handle: {game.Handle:X} }}");

var namesAddress = PatternScanner.Search(game.Handle, game.MainModule, "E8 [....] 48 C7 C7", (bytes, address) =>
{
...
var objectsAddress = PatternScanner.Search(game.Handle, game.MainModule, "E8 [....] 8B 5D AF", (bytes, address) =>
{
    address += BitConverter.ToInt32(bytes, address) + 0x4 + 0x65;
    address += BitConverter.ToInt32(bytes, address + 0x3) + 0x7;
    return address;
});
